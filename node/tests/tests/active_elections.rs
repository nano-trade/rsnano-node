...
fn bound_election_winners() {
...

    {
        ...
        for block in blocks {
            node.vote_applier.force_confirm(&block.hash());
        }
...
fn conflicting_block_vote_existing_election() {
    ...
    assert_timely2(|| node.active.is_active_root(&fork.qualified_root()));
    assert_timely2(|| {
        node.active
            .read()
...
fn activate_account_chain() {
    ...
    node.vote_applier.force_confirm(&open.hash());
    ...
    node.vote_applier.force_confirm(&send3.hash());
